...
class ObjectRemover:
    ...
    def __call__(self, raw_image, raw_mask):
        ...
        with tf.Session() as sess:
            ...
            return self._inpaint(
                raw_image, raw_mask, sess, inpainted_512_node, attention_node, mask_512_node, image_ph, mask_ph, 6
            )
    ...
    def _inpaint(
        ...
        inpainted_512, attention, mask_512 = sess.run(
            [inpainted_512_node, attention_node, mask_512_node],
            feed_dict={img_512_ph: [img_512], mask_512_ph: [mask_512]},
        )

        # post-processing
        res_raw_size = self._post_process(
            raw_img, img_large, mask_large, inpainted_512[0], img_512, mask_512[0], attention[0], multiple
        )
        return res_raw_size
    ...
    def _pre_process(self, raw_img, raw_mask, multiple):
        ...
        large_mask = cv2.resize(
            raw_mask, (multiple * INPUT_SIZE, multiple * INPUT_SIZE), interpolation=cv2.INTER_NEAREST
        )
    ...
    def _post_process(self, raw_img, large_img, large_mask, res_512, img_512, mask_512, attention, multiple):
        ...
        h, w, c = raw_img.shape
        low_base = cv2.resize(
            res_512.astype(np.float32), (INPUT_SIZE * multiple, INPUT_SIZE * multiple), interpolation=cv2.INTER_LINEAR
        )
        low_large = cv2.resize(
            img_512.astype(np.float32), (INPUT_SIZE * multiple, INPUT_SIZE * multiple), interpolation=cv2.INTER_LINEAR
        )
